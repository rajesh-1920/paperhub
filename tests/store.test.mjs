...
test("phAddFile adds to global + owner files and queues a review, then persists to the server", () => {
  const { window, user, server } = bootPage(USER_PAGE, ["utils.js"], "user");
  ...
  // Persisted to the (in-memory) backend via PUT /api/dataset.
  assert.ok(
    server.dataset.files.some((f) => f.id === "file-test-1"),
    "dataset persisted to the server",
  );
});
...
test("mutations persist across a reload via the server", () => {
  const shared = {};
  const a = bootPage(USER_PAGE, ["utils.js"], "user", shared);
  a.window.phUpdateUser("user-mahmud.hasan", { name: "Persisted" });

  // A fresh page load (same backend) reads the persisted dataset, not the seed.
  const b = bootPage(USER_PAGE, ["utils.js"], "user", shared);
  assert.equal(
...
test("resetPaperHubData restores the dataset from the seed", () => {
  const shared = {};
  const a = bootPage(USER_PAGE, ["utils.js"], "user", shared);
  a.window.phUpdateUser("user-mahmud.hasan", { name: "Mutated" });
  a.window.resetPaperHubData();
